...
export default function RecetaComp({
	...
	const diamonds = Array.from({length: Math.max(0, Math.min(4, difficulty))})
		...
		return (
			<Link to="/RecetaPg" className="no-underline">
				<article className={`bg-[#FFF8E1] border-2 border-gray-900 rounded-lg overflow-hidden flex flex-col sm:flex-row items-stretch gap-4 p-4 ${className}`}>
