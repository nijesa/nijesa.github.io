...
// Home is rendered inside SharedLayout which already provides ResponsiveLayout + TopBar
import useStore from '../store'

export default function Home(){
    // use global menu state so TopBar can toggle filters
    ...
    return (
        <div className="min-h-[60vh]">
            <div className="md:gap-8 md:items-start">
                {/* Main content - fluid on desktop */}
                <main className="flex-1">
                                    {/* Overlay: only on mobile */}
                                    <div className={`fixed inset-0 z-40 transform transition-transform duration-300 md:hidden ${menuOpen ? 'translate-y-0' : '-translate-y-full'}`}>
                                        <div className="h-full bg-card p-4">
                                            <div className="flex items-center justify-between mb-4">
                                                <button aria-label="close menu" onClick={()=>setMenuOpen(false)} className="p-1">
                                                    <svg className="w-6 h-6 text-[var(--text)]" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    {/* Content list: full width on desktop, padded */}
                                    <div className="p-4 md:p-0" style={{ backgroundColor: 'var(--bg)' }}>
